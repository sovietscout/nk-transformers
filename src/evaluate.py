...
def evaluate_one_step_mse(model, data: dict, stats: dict, device: str = "cuda"):
    ...
    start_t = 50
    overall, per_var = compute_mse(
        pred_unnorm[:, start_t:, :], true_unnorm[:, start_t:, :]
    )
    return overall, per_var
...
def evaluate_var_one_step(data: dict, stats: dict):
    ...
    with Pool(cpu_count()) as pool:
        results = list(
            tqdm(
                pool.imap(
                    _var_one_step_worker, [(Y_test_raw[i], p_max) for i in range(N)]
                ),
                total=N,
                desc="VAR one-step MSE",
            )
        )
    ...
    start_t = 50
    overall, per_var = compute_mse(
        all_preds[:, start_t:, :], Y_test_raw[:, start_t:, :]
    )
    return overall, per_var
...
def evaluate_bvar_one_step(data: dict, stats: dict):
    ...
    with Pool(cpu_count()) as pool:
        results = list(
            tqdm(
                pool.imap(_bvar_one_step_worker, [Y_test_raw[i] for i in range(N)]),
                total=N,
                desc="BVAR one-step MSE",
            )
        )
...
def evaluate_multistep_mse(
    ...
    for h in horizons:
        ...
        for i in tqdm(range(0, N, batch_size), desc=f"Multi-step h={h}"):
            ...
            future_shocks = torch.from_numpy(
                X_test[i:end_i, context_end : context_end + h, 11:14]
            ).to(device)
            ...
            pred_norm = model.autoregressive_forecast(
                x_context, horizon=h, future_shocks=future_shocks, true_lag=true_lag
            )
...
def evaluate_var_multistep(data: dict, stats: dict, horizons: list = [1, 4, 8, 12, 20]):
    ...
    for h in horizons:
        with Pool(cpu_count()) as pool:
            res = list(
                tqdm(
                    pool.imap(_var_ms_worker, [(Y_test_raw[i], h) for i in range(N)]),
                    total=N,
                    desc=f"VAR multi-step h={h}",
                )
            )
...
def evaluate_bvar_multistep(
    data: dict, stats: dict, horizons: list = [1, 4, 8, 12, 20]
):
    ...
    for h in horizons:
        with Pool(cpu_count()) as pool:
            res = list(
                tqdm(
                    pool.imap(_bvar_ms_worker, [(Y_test_raw[i], h) for i in range(N)]),
                    total=N,
                    desc=f"BVAR multi-step h={h}",
                )
            )
        all_preds = np.stack([r[0] for r in res])
        all_trues = np.stack([r[1] for r in res])
...
def compute_transformer_path_batch(
    model,
    params_raw: np.ndarray,
    shocks_raw: np.ndarray,
    stats: dict,
    device: str = "cuda",
):
    ...
    with torch.no_grad():
        for h in range(H):
            ...
            inp = torch.cat(
                [params_norm_t, shocks_norm_t[:, h : h + 1, :], lag_norm_t], dim=-1
            )
...
def compute_transformer_irf_batch(
    model,
    params_raw: np.ndarray,
    shock_vecs: np.ndarray,
    stats: dict,
    horizon: int,
    device: str = "cuda",
):
    ...
    path_shocked = compute_transformer_path_batch(
        model, params_raw, shocks_shocked, stats, device=device
    )
    path_baseline = compute_transformer_path_batch(
        model, params_raw, shocks_baseline, stats, device=device
    )
...
def _irf_accuracy_worker(args):
    ...
    local_results = {
        m: {
            s: {v: {"mse": None, "sign_acc": None} for v in var_names}
            for s in ["r", "u", "v"]
        }
        for m in ["VAR", "BVAR"]
    }
    ...
    for s_idx, shock_type in enumerate(["r", "u", "v"]):
        ...
        try:
            ...
            var_irf_result = var_irf(
                B, Sigma, p, horizon=20, shock_index=s_idx, shock_size=1.0
            )
        except Exception:
            var_irf_result = np.zeros((21, 3))
        try:
            bvar_res = bvar_minnesota_fit(Y_i, p=4)
            bvar_irf_mean, _, _ = bvar_irf(
                bvar_res, horizon=20, shock_index=s_idx, shock_size=1.0
            )
        except Exception:
        ...
        for m_name, pred_irf in [("VAR", var_irf_result), ("BVAR", bvar_irf_mean)]:
            for v_idx, v_name in enumerate(var_names):
                ...
                mask = true_sign != 0
                sign_acc = (
                    (true_sign[mask] == pred_sign[mask]).mean()
                    if mask.sum() > 0
                    else 1.0
                )
                local_results[m_name][shock_type][v_name] = {
                    "mse": mse,
                    "sign_acc": sign_acc,
                }
...
def evaluate_irf_accuracy(
    model, data: dict, stats: dict, n_sims: int = 200, device: str = "cuda"
):
    ...
    with Pool(cpu_count()) as pool:
        benchmark_res = list(
            tqdm(
                ...
                total=n_sims,
                desc="IRF evaluation (Benchmarks)",
            )
        )

    results = {
        m: {s: {v: {"mse": [], "sign_acc": []} for v in var_names} for s in shock_types}
    ...
    for s_idx, shock_type in enumerate(shock_types):
        all_shock_vecs = np.stack(
            [benchmark_res[i][2][shock_type] for i in range(n_sims)]
        )
        tf_irfs = compute_transformer_irf_batch(
            model,
            params_test_raw[:n_sims],
            all_shock_vecs,
            stats,
            horizon=20,
            device=str(device),
...
def evaluate_kalman_one_step(data: dict):
    ...
    with Pool(cpu_count()) as pool:
        results = list(
            tqdm(
                pool.imap(_kalman_worker, [(Y_test_raw[i], start_t) for i in range(N)]),
                total=N,
                desc="Kalman one-step MSE",
            )
        )
    all_preds = np.stack(results)
    overall, per_var = compute_mse(
...
def collect_irf_paths(
    ...
    for s_idx, shock_type in enumerate(["r", "u", "v"]):
        ...
        tf_irf = compute_transformer_irf_batch(
            model,
            params_i[None, :],
            shock_vec[None, :],
            stats,
        ...
        try:
            p, B, c, Sigma, _ = select_var_order(Y_i, p_max=8)
            var_path = var_irf(
                B, Sigma, p, horizon=horizon, shock_index=s_idx, shock_size=1.0
            )
        except Exception:
            var_path = np.zeros_like(true_irf)
        try:
            bvar_res = bvar_minnesota_fit(Y_i, p=4)
            bvar_path, _, _ = bvar_irf(
                bvar_res, horizon=horizon, shock_index=s_idx, shock_size=1.0
            )
        except Exception:
