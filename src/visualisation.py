...
def plot_trajectory_overlay(
    ...
    for ax, obs_idx, name in zip(axes, range(3), OBS_NAMES):
        ax.plot(
            ...
            linewidth=LINEWIDTHS["True"],
            label="True",
        )
        ax.plot(
            range(T),
            pred[:, obs_idx],
            color=COLOURS["Transformer"],
            linestyle=LINESTYLES["Transformer"],
            linewidth=LINEWIDTHS["Transformer"],
            label="Transformer",
        )
...
def plot_all_model_trajectories(
    data: dict,
    stats: dict,
    model,
    var_results: dict,
    ...
    fig, axes = plt.subplots(
        len(sim_indices),
        3,
        figsize=(18, 5.4 * len(sim_indices)),
    ...
    for row, sim_idx in enumerate(sim_indices):
        ...
        var_pred = np.array(
            var_results.get("predictions", {}).get(sim_idx, np.zeros((T, 3)))
        )
        bvar_pred = np.array(
            bvar_results.get("predictions", {}).get(sim_idx, np.zeros((T, 3)))
        )
        ...
        for col, (obs_idx, name) in enumerate(zip(range(3), OBS_SHORT)):
            ...
            if bvar_pred.any():
                ax.plot(
                    range(T),
                    bvar_pred[:, obs_idx],
                    linestyle=LINESTYLES["BVAR"],
                    color=COLOURS["BVAR"],
                    linewidth=LINEWIDTHS["BVAR"],
                    label="BVAR",
                )
...
def plot_reduced_form_trajectory_overlay(
    data: dict,
    stats: dict,
    model,
    save_path: str,
    ...
    for s_row, (shock_type, shock_label) in enumerate(shock_labels.items()):
        for v_col, v_name in enumerate(var_names):
            ...
            for m_name in model_names:
                if (
                    shock_type in irf_results.get(m_name, {})
                    and v_name in irf_results[m_name][shock_type]
                ):
                    val = irf_results[m_name][shock_type][v_name]["irf_mse_mean"]
                    vals.append(val)
                    ax.bar(
                        model_names.index(m_name),
                        val,
                        color=COLOURS[m_name],
                        alpha=0.88,
                        width=0.62,
...
def plot_irf_paths(irf_paths: dict, save_path: str):
    ...
    fig, axes = plt.subplots(
        3, 3, figsize=(17, 13.5), sharex=True, constrained_layout=True
    )
    for row, (shock_type, shock_label) in enumerate(shock_labels.items()):
        for col, obs_name in enumerate(OBS_SHORT):
            ...
            format_axis(
                ax,
                xlabel="Quarter" if row == 2 else None,
...
def plot_learning_curve(
    sample_sizes: list, tf_mse: list, var_mse: list, bvar_mse: list, save_path: str
):
...
def plot_forecast_horizon(mses: dict, save_path: str):
    ...
    for m_name, mse_dict in mses.items():
        ...
        values = [
            mse_dict[h][0] if isinstance(mse_dict[h], tuple) else mse_dict[h]
            for h in horizons
        ]
        ax.semilogy(
            horizons,
