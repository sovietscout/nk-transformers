...
def load_config(config_path: str) -> Config:
    ...
    if "device" not in cfg_dict:
        raise KeyError("Missing required config key: device")

    return Config.from_toml(cfg_dict)
...
def _main(cfg, total_t0):
    ...
    var_baseline = (
        one_step_results.get("VAR", (np.nan,))[0]
        if "VAR" in one_step_results
        else np.nan
    )
    bvar_baseline = (
        one_step_results.get("BVAR", (np.nan,))[0]
        if "BVAR" in one_step_results
        else np.nan
    )
    ...
    experiment_config = {
        "training": {k: v for k, v in asdict(cfg.training).items() if k != "compile"},
